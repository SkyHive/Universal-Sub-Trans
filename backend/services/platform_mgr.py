    ...
    def setup_runtime_env() -> None:
        ...
        if vendor == "nvidia":
            if dep_mgr.check_missing_deps():
                logger.warning(f"missing_cuda_dependencies on {current_os}: GPU acceleration might not be available until installed.")
                ...
                try:
                    ...
                    if current_os == "windows":
                        ...
                        os.environ["LD_LIBRARY_PATH"] = abs_lib_path + (os.pathsep + old_ld_path if old_ld_path else "")
